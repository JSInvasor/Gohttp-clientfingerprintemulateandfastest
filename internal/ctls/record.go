...
// writeRawRecord writes a TLS record carrying the legacy_record_version every
// record other than an initial ClientHello must use.
//
// RFC 8446 §5.1: legacy_record_version "MUST be set to 0x0303 for all records
// generated by a TLS 1.3 implementation other than an initial ClientHello
// (i.e., one not generated after a HelloRetryRequest), where it MAY also be
// 0x0301 for compatibility purposes."
//
// The 0x0301 exemption is narrow, and reading it as "every handshake record"
// breaks the retry flight specifically: the second ClientHello is a handshake
// record that is NOT an initial ClientHello, and a server that has already
// pinned the version rejects it. Go's crypto/tls does exactly that, with
// "received record with version 301 when expecting version 303" — answered on
// the wire as a protocol_version alert.
func writeRawRecord(conn net.Conn, typ uint8, data []byte) error {
	return writeRecordVersion(conn, typ, versionTLS12, data)
}

// writeInitialClientHello writes the one record allowed to carry 0x0301. Both
// emulated browsers use the compatibility version here, so this is a
// fingerprint detail as much as a protocol one.
func writeInitialClientHello(conn net.Conn, data []byte) error {
	return writeRecordVersion(conn, recordTypeHandshake, versionTLS10, data)
}
