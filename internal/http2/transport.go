...
type Transport struct {
	...
	// If set, pseudo-headers are written in this order instead of the default
	// (:authority, :method, :path, :scheme).
	// Example for Firefox 148: []string{":method", ":path", ":authority", ":scheme"}
	PseudoHeaderOrder []string

	// HeaderOrder specifies the order of regular HTTP headers.
	// If set, headers matching these names are written first in this order,
	// followed by any remaining headers in their original order.
	// Names should be in canonical form (e.g., "User-Agent", "Accept").
	HeaderOrder []string

	// Settings specifies the exact SETTINGS to send in the initial frame.
	// If set, these settings are sent instead of the auto-generated defaults.
	// The order of settings is preserved on the wire, matching browser fingerprints.
	// Each Setting has an ID (e.g., SettingHeaderTableSize) and Val.
	Settings []Setting

	// ConnectionFlow specifies the connection-level WINDOW_UPDATE increment
	// sent after the initial SETTINGS frame. If 0, the default (1<<30) is used.
...
func (t *Transport) newClientConn(c net.Conn, singleUse bool) (*ClientConn, error) {
	...
	// Use custom settings if provided (for browser fingerprint emulation),
	// otherwise use the default auto-generated settings.
	...
	// Use custom connection flow if provided, otherwise use default.
...
func (cc *ClientConn) encodeHeaders(req *http.Request, addGzipHeader bool, trailers string, contentLength int64) ([]byte, error) {
	...
	enumerateHeaders := func(f func(name, value string)) {
		...
		if len(cc.t.PseudoHeaderOrder) > 0 {
			...
			// Default Go order: :authority, :method, :path, :scheme
			f(":authority", host)
			f(":method", m)
			if !isNormalConnect(req) {
				f(":path", path)
		...
		// emitHeader handles special-case headers (user-agent, cookie, etc.)
		var didUA bool
		emitHeader := func(k string, vv []string) {
		...
		if len(cc.t.HeaderOrder) > 0 {
			...
			// Second pass: emit remaining headers not in the order list
			for k, vv := range req.Header {
				if !emitted[k] {
					emitHeader(k, vv)
				}
			...
			// Default: iterate in map order (random)
			for k, vv := range req.Header {
				emitHeader(k, vv)
			}
		}
